...
totaldata <- fromJSON("/Users/Sammy/Dropbox/project/code/clean_data/featurized1.json")
totaldata$userscore <- NULL

View(totaldata)

## baseline
# summary(glm(ROI ~ runtime + G + PG + PG_13 + R + 
...
set.seed(1000)
rands <- sample(3698,2894)
train2data <- totaldata[rands,]
test2data <- totaldata[-rands,]

## use training data to fit model, first training set first
glm.1 <- glm(ROI ~ runtime + G + PG + PG_13 + R + 
               Action + Adventure + Animation + Biography + Comedy + Crime + Drama + 
               Family + Fantasy + History + Horror + Music + Musical + Mystery + 
               Romance + SciFi + Short + Sport + Thriller + War + Western + 
               January + February + March + April + May + June + July + August + September + October + November + December, 
             data=train1data,family=gaussian)
glm.train.1 <- mean(abs(predict(glm.1,train1data) - train1data$ROI))
glm.train.1
glm.test.1 <- mean(abs(predict(glm.1,test1data) - test1data$ROI))
glm.test.1
...
## random train/test
glm.2 <- glm(ROI ~ runtime + G + PG + PG_13 + R + 
               Action + Adventure + Animation + Biography + Comedy + Crime + Drama + 
               Family + Fantasy + History + Horror + Music + Musical + Mystery + 
               Romance + SciFi + Short + Sport + Thriller + War + Western + 
               January + February + March + April + May + June + July + August + September + October + November + December, 
             data=train2data,family=gaussian)
glm.train.2 <- mean(abs(predict(glm.2,train2data) - train2data$ROI))
glm.train.2
glm.test.2 <- mean(abs(predict(glm.2,test2data) - test2data$ROI))
glm.test.2
## lasso and ridge
# compute the ridge path
...
######## GAMMA STUFF ########
glm.3 <- glm(ROI ~ runtime + G + PG + PG_13 + R + 
               Action + Adventure + Animation + Biography + Comedy + Crime + Drama + 
               Family + Fantasy + History + Horror + Music + Musical + Mystery + 
               Romance + SciFi + Short + Sport + Thriller + War + Western + 
               January + February + March + April + May + June + July + August + September + October + November + December, 
...
coefs <- coef(glm.3)
glm.train.3 <- mean(abs(predict(glm.3,train1data) - train1data$ROI))
predict(glm.3,train1data) - train1data
glm.train.3
glm.test.3 <- mean(abs(predict(glm.3,test1data) - test1data$ROI))
glm.test.3
## lasso and ridge
# compute the ridge path
...
train <- fromJSON("/Users/Sammy/Dropbox/project/code/clean_data/featurized3_train.json")
test <- fromJSON("/Users/Sammy/Dropbox/project/code/clean_data/featurized3_test.json")
glm.1 <- glm(ROI ~ runtime + G + PG + PG_13 + R + 
               Action + Adventure + Animation + Biography + Comedy + Crime + Drama + 
               Family + Fantasy + History + Horror + Music + Musical + Mystery + 
               Romance + SciFi + Short + Sport + Thriller + War + Western + 
...
glm.train.1 <- mean(abs(predict(glm.1,train) - train$ROI))
glm.train.1
glm.test.1 <- mean(abs(predict(glm.1,test) - test$ROI))
glm.test.1
## lasso and ridge
# compute the ridge path
train.x <- as.matrix(train[,c(1:29,31:41,43,46,48,49)],header=T)
train.y <- train[,30]
test.x <- as.matrix(test[,c(1:29,31:41,43,46,48,49)],header=T)
test.y <- test[,30]
ridge.model <- glmnet(train.x, train.y, family="gaussian", alpha=0)
# plot the ridge path
plot(ridge.model)
